...
public class PageDao {
	...
	public static Page init(Page page, Hashtable<String, String> cfgs) {
		for (String str : cfgs.keySet()) {
			if (str.startsWith("data."))
				...
				if (idx > 0 && idx < key.length() - 1) {
					page.addAreaMap(key.substring(0, idx).trim(), key.substring(idx + 1).trim(), cfgs.get(str));
				}
			}
		}
		...
		page.setTemplatePrefixes(BranchDao.parseTemplatePrefix(cfgs.get("template-prefix")));
		...
		page.setStatic("".equals(cfgs.get("static")) || "true".equalsIgnoreCase(cfgs.get("static")));
