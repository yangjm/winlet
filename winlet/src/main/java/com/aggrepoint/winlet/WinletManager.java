...
public class WinletManager implements WinletConst {
	...
	static Hashtable<Object, Hashtable<String, Hashtable<String, FormImpl>>> getFormsByWinlet(
			HttpSession session) {
		synchronized (session) {
			@SuppressWarnings("unchecked")
			Hashtable<Object, Hashtable<String, Hashtable<String, FormImpl>>> ht = (Hashtable<Object, Hashtable<String, Hashtable<String, FormImpl>>>) session
					.getAttribute(FORM_BY_WIDGET_SESSION_KEY);

			if (ht == null) {
				ht = new Hashtable<Object, Hashtable<String, Hashtable<String, FormImpl>>>();
				session.setAttribute(FORM_BY_WIDGET_SESSION_KEY, ht);
			}
	...
	static Hashtable<String, FormImpl> getFormsById(HttpSession session) {
		synchronized (session) {
			@SuppressWarnings("unchecked")
			Hashtable<String, FormImpl> ht = (Hashtable<String, FormImpl>) session
					.getAttribute(FORM_BY_ID_SESSION_KEY);

			if (ht == null) {
				ht = new Hashtable<String, FormImpl>();
				session.setAttribute(FORM_BY_ID_SESSION_KEY, ht);
			}
	 ...
	 * @return
	 */
	public static FormImpl getForm(ReqInfo req, String formName) {
	 ...
	 * @return
	 */
	public static FormImpl getForm(ReqInfo reqInfo) {
		if (reqInfo.getFormId() == null)
			return null;

		return getFormsById(reqInfo.getSession()).get(reqInfo.getFormId());
	}
